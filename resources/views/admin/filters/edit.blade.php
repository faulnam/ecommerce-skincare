...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">
        <i class="fas fa-edit me-2"></i>Edit Filter
    </h4>
    ...
    <div class="card-body">
        <form action="{{ route('admin.filters.update', $filter) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('category') is-invalid @enderror" id="category" name="category" required onchange="toggleNewCategory(this)">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat }}" {{ old('category', $filter->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $filter->name) }}" required placeholder="Contoh: Merah, XL, Nox, dll">
                @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $filter->is_active) ? 'checked' : '' }}>
