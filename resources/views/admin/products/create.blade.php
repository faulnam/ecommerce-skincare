                ...
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  id="description" name="description" rows="3" placeholder="Deskripsi umum produk">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                                        ...
                                        <select class="form-select @error($field['name']) is-invalid @enderror" id="{{ $field['name'] }}" name="{{ $field['name'] }}">
                                            ...
                                            @if(isset($specifications[$field['category']]))
                                                @foreach($specifications[$field['category']] as $spec)
                                                    <option value="{{ $spec->name }}" {{ old($field['name']) == $spec->name ? 'selected' : '' }}>
                                                        {{ $spec->name }}
                                                    </option>
                                                @endforeach
                                            @endif
                    ...
                    <div class="card border-warning mb-3 position-relative" id="mainProductDiscountSection">
                        ...
                        <div class="card-body">
                            ...
                            <small class="text-muted">Kosongkan tanggal jika diskon berlaku selamanya. (Jika produk memiliki varian, tanggal akan diterapkan ke semua varian, sedangkan persentase diskon per varian diatur di bawah).</small>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3" id="bundleTypeContainer" style="display: {{ old('package_type') == 'bundle' ? 'block' : 'none' }};">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('package_weight') is-invalid @enderror" id="package_weight" name="package_weight" value="{{ old('package_weight', 500) }}" min="0">
                    ...
                    <div class="mb-4 position-relative" id="mainProductImageSection">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control @error('image') is-invalid @enderror" 
                                   id="image" name="image" accept="image/*" onchange="previewImage(this, 1)" required>
                            @error('image')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <div id="variantsSection" style="display:{{ old('has_variants') ? 'block' : 'none' }}">
                <div class="card border-primary mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <p id="variantsEmpty" class="text-muted text-center py-3">Belum ada varian.</p>
                    </div>
                </div>
            </div>
...
function previewImage(input, imageNumber) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = e => preview.innerHTML = `<img src="${e.target.result}" class="img-fluid rounded border" style="max-height:150px;">`;
    ...
    div.innerHTML = `
        ...
        <div class="row g-2">
            ...
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <span class="input-group-text">Rp</span>
                    <input type="number" class="form-control" name="variants[${i}][price]" required min="0" step="100" oninput="updateVariantPreview(this)">
                </div>
            </div>
            <div class="col-md-2">
...
function updateVariantPreview(discountInput) {
    const row = discountInput.closest('.variant-item');
