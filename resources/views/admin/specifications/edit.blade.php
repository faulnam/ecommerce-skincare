...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">
        <i class="fas fa-edit me-2"></i>Edit Spesifikasi
    </h4>
    ...
    <div class="card-body">
        <form action="{{ route('admin.specifications.update', $specification) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('category') is-invalid @enderror" id="category" name="category" required onchange="toggleNewCategory(this)">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat }}" {{ old('category', $specification->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $specification->name) }}" required placeholder="Contoh: Merah, XL, Nox, dll">
                @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $specification->is_active) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-end">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-save me-1"></i>Update Spesifikasi
                </button>
            </div>
        </form>
    </div>
